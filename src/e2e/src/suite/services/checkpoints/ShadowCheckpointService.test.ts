...
describe.each([
	...
	setup(async () => {
		jest.mocked(globby).mockClear().resolves([])
	...
	suite(`${klass.name}#renameNestedGitRepos`, () => {
		test("handles nested git repositories during initialization", async () => {
			...
			jest.mocked(globby).callsFake((pattern: string | readonly string[]) => {
				if (pattern === "**/.git") {
			...
			const renameCallsArgs = renameSpy.args.map((call) => String(call[0]) + " -> " + String(call[1]))
			expect(
	...
	suite(`${klass.name}#events`, () => {
		test("emits initialize event when service is created", async () => {
			...
			for (let i = 0; i < emitSpy.args.length; i++) {
				const call = emitSpy.args[i]
		...
		test("emits checkpoint event when saving checkpoint", async () => {
			...
			assert.ok(result?.commit !== undefined)

			assert.strictEqual(checkpointHandler.callCount, 1)
			const eventData = checkpointHandler.args[0][0]
			assert.strictEqual(eventData.type, "checkpoint")
			assert.ok(eventData.toHash !== undefined)
		...
		test("emits restore event when restoring checkpoint", async () => {
			...
			const eventData = restoreHandler.args[0][0]
		...
		test("emits error event when an error occurs", async () => {
			...
			const eventData = errorHandler.args[0][0]
		...
		test("supports multiple event listeners for the same event", async () => {
			...
			const eventData1 = checkpointHandler1.args[0][0]
			const eventData2 = checkpointHandler2.args[0][0]
