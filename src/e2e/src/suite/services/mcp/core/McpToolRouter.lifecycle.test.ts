...
suite("McpToolRouter - Lifecycle and Event Management", () => {
	...
	let mockExecutor: sinon.SinonStubStatic<McpToolExecutor>
	...
	setup(() => {
		...
		mockExecutor = {
			...
			on: sinon.stub().returns((event: string, handler: Function) => {
				executorEventEmitter.on(event, handler as any)
				return mockExecutor
			}),
			emit: sinon.stub().returns((event: string, ...args: any[]) => {
				executorEventEmitter.emit(event, ...args)
				return true
			}),
			removeAllListeners: sinon.stub().returns(() => {
	...
	suite("Error Handling", () => {
		...
		test("should continue forwarding events after errors", async () => {
			...
			mockExecutor.initialize.rejectsOnce(new Error("Init error"))
