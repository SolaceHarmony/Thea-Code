...
suite("port-utils - Retry and Timeout Tests", () => {
	...
	let mockLogger: sinon.SinonStubStatic<typeof logger>
	...
	setup(() => {
		...
		// Get mocked module
		mockTcpPortUsed = // TODO: requireMock("tcp-port-used")
		mockLogger = logger as sinon.SinonStubStatic<typeof logger>
		...
		if (typeof // TODO: global jest reference !== 'undefined') {
			delete // TODO: global jest reference
	...
	teardown(() => {
		...
		// Restore global jest
		// TODO: global jest reference = jest
	...
	suite("findAvailablePort", () => {
		test("should retry up to maxAttempts times", async () => {
			...
			assert.ok(mockTcpPortUsed.check.args.length >= 5)
		...
		test("should switch to random port search after 20 sequential attempts", async () => {
			...
			assert.ok(consoleLogSpy.calledWith(
				sinon.match.string.and(sinon.match("Switching to random port search")))
		...
		test("should handle preferred ranges with retries", async () => {
			...
			assert.ok(consoleLogSpy.calledWith(
				sinon.match.string.and(sinon.match("Found available port")))
		...
		test("should validate and skip invalid port ranges", async () => {
			...
			assert.ok(consoleWarnSpy.calledWith(
				sinon.match.string.and(sinon.match("Invalid port range")))
	...
	suite("waitForPortAvailable", () => {
		test("should retry with exponential backoff", async () => {
			...
			assert.ok(mockLogger.warn.calledWith(
				sinon.match.string.and(sinon.match("Retry 1/5"))),
				sinon.match.object

			assert.ok(mockLogger.warn.calledWith(
				sinon.match.string.and(sinon.match("Retry 2/5"))),
				sinon.match.object
		...
		test("should throw after maxRetries exceeded", async () => {
			...
			assert.ok(mockLogger.error.calledWith(
				sinon.match.string.and(sinon.match("Timeout waiting"))),
				sinon.match.object
	...
	suite("waitForPortInUse", () => {
		...
		test("should skip in test environment with global jest", async () => {
			// TODO: global jest reference = jest
	...
	suite("Edge Cases", () => {
		test("should handle network timeouts gracefully", async () => {
			...
			assert.strictEqual(result, false)
			assert.ok(consoleErrorSpy.calledWith(
				sinon.match.string.and(sinon.match("Error checking port"))),
				expect.any(Error)
		...
		test("should handle concurrent retry operations", async () => {
			...
			assert.ok(mockLogger.info.calledWith(
				sinon.match.string.and(sinon.match("server1 on port 3000 is now ready"))),
				sinon.match.object

			assert.ok(mockLogger.info.calledWith(
				sinon.match.string.and(sinon.match("server2 on port 3001 is now ready"))),
				sinon.match.object
