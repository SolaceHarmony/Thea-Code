...
const mockVscode = {
	...
	Uri: {
		parse: sinon.stub().returns((url: string) => createMockUri("https", url)),
		file: sinon.stub().returns((path: string) => createMockUri("file", path)),
	},
	Position: sinon.stub(),
	Range: sinon.stub(),
	TextEdit: sinon.stub(),
...
suite("mentions", () => {
	...
	setup(() => {
		...
		mockVscode.workspace.openTextDocument.mockReset().resolves({})
		mockVscode.window.showTextDocument.mockReset().resolves({})
...
- Added tests`
			...
			jest.mocked(git.getCommitInfo).resolves(commitInfo)
		...
		test("should handle errors fetching git info", async () => {
			...
			jest.mocked(git.getCommitInfo).rejects(new Error(errorMessage))
	...
	suite("openMention", () => {
		test("should handle file paths and problems", async () => {
			...
			mockVscode.workspace.fs.stat.rejectsOnce(new Error("File does not exist"))
		...
		test("should handle URLs", async () => {
			...
			const calledArg = mockVscode.env.openExternal.args[0][0]
