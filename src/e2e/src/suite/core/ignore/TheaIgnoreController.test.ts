...
suite(`${AI_IDENTITY_NAME}Ignore Controller`, () => {
	...
	interface MockFileWatcher {
		onDidCreate: sinon.SinonStub Promise<void> | void) => { dispose: () => void }>
		onDidChange: sinon.SinonStub Promise<void> | void) => { dispose: () => void }>
		onDidDelete: sinon.SinonStub Promise<void> | void) => { dispose: () => void }>
	...
	setup(() => {
		...
		mockReadFile = fs.readFile as sinon.SinonStub
	...
	suite("file watcher", () => {
		...
		test(`should reload ${GLOBAL_FILENAMES.IGNORE_FILENAME} when file is created`, async () => {
			...
			mockFileExists.mockReset().resolves(true)
			mockReadFile.mockReset().resolves("node_modules")
		...
		test(`should reload ${GLOBAL_FILENAMES.IGNORE_FILENAME} when file is changed`, async () => {
			...
			const onChangeHandler = mockWatcher.onDidChange.args[0][0]
		...
		test(`should reset when ${GLOBAL_FILENAMES.IGNORE_FILENAME} is deleted`, async () => {
			...
			const onDeleteHandler = mockWatcher.onDidDelete.args[0][0]
