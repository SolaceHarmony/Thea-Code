...
/*
=> {
	const originalPath = // TODO: requireActual<typeof import("path")>("path")
	return {
		...originalPath,
...
suite("read_file tool with maxReadFileLine setting", () => {
	...
	const mockedCountFileLines = countFileLines as sinon.SinonStub
	const mockedReadLines = readLines as sinon.SinonStub
	const mockedExtractTextFromFile = extractTextFromFile as sinon.SinonStub
	const mockedAddLineNumbers = addLineNumbers as sinon.SinonStub
	const mockedParseSourceCodeDefinitionsForFile = parseSourceCodeDefinitionsForFile as sinon.SinonStub
	const mockedIsBinaryFile = isBinaryFile as sinon.SinonStub
	// Use bind to avoid unbound method reference
	const mockedPathResolve = path.resolve.bind(path) as sinon.SinonStub
