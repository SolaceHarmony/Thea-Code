...
suite("askFollowupQuestionTool", () => {
	...
	const mockedFormatResponse = formatResponse as sinon.SinonStubStatic<typeof formatResponse>
	...
	setup(() => {
		...
		mockRemoveClosingTag = sinon.stub().returns((tag: string, content?: string) => content ?? "") as RemoveClosingTag
