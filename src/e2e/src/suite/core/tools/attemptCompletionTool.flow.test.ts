...
suite("attemptCompletionTool - Partial/Final Flow Tests", () => {
	let mockTheaTask: sinon.SinonStubStatic<TheaTask>
	...
	let mockToolDescription: sinon.SinonStub
	let mockAskFinishSubTaskApproval: sinon.SinonStub
	let mockTelemetryService: sinon.SinonStubStatic<typeof telemetryService>
	...
	setup(() => {
		...
		mockRemoveClosingTag = sinon.stub().returns((tag, content) => content || "")
		mockToolDescription = sinon.stub().returns("Attempt Completion")
		mockAskFinishSubTaskApproval = sinon.stub().resolves(true)

		// Mock telemetry service
		mockTelemetryService = telemetryService as sinon.SinonStubStatic<typeof telemetryService>
	...
	suite("User Feedback Flow", () => {
		...
		test("should include command result in feedback flow", async () => {
			...
			assert.ok(mockTheaTask.userMessageContent.some(item => JSON.stringify(item) === JSON.stringify({
				type: "text",
				text: "Tests passed"
			})))
