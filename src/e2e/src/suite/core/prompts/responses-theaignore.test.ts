...
suite("TheaIgnore Response Formatting", () => {
	...
	suite("formatResponse.formatFilesList with TheaIgnoreController", () => {
		...
		test("should format files list with lock symbols for ignored files", async () => {
			...
			assert.ok(result.match(new RegExp(`${LOCK_TEXT_SYMBOL}.*node_modules/package.json`, "i")))
			assert.ok(result.match(new RegExp(`${LOCK_TEXT_SYMBOL}.*\\.git/HEAD`, "i")))
			assert.ok(result.match(new RegExp(`${LOCK_TEXT_SYMBOL}.*secrets/keys.json`, "i")))
		...
		test("should handle truncation with TheaIgnoreController", async () => {
			...
			assert.ok(result.match(/use list_files on specific subdirectories/i))
