...
suite("importExport", () => {
	...
	let mockExtensionContext: sinon.SinonStubStatic<vscode.ExtensionContext>
	...
	setup(() => {
		...
		} as unknown as sinon.SinonStubStatic<ProviderSettingsManager>
		...
		} as unknown as sinon.SinonStubStatic<vscode.ExtensionContext>
	...
	suite("importSettings", () => {
		...
		test("should not clobber existing api configs", async () => {
			...
			assert.strictEqual(result.success, true)
			assert.ok(result.providerProfiles?.apiConfigs["openai"] !== undefined)
			assert.ok(result.providerProfiles?.apiConfigs["default"] !== undefined)
