...
suite("ContextProxy", () => {
	...
	suite("constructor", () => {
		test("should initialize state cache with all global state keys", () => {
			assert.strictEqual(mockGlobalState.get.callCount, GLOBAL_STATE_KEYS.length)
			for (const key of GLOBAL_STATE_KEYS) {
				assert.ok(mockGlobalState.get.calledWith(key))

		test("should initialize secret cache with all secret keys", () => {
			assert.strictEqual(mockSecrets.get.callCount, SECRET_STATE_KEYS.length)
	...
	suite("getGlobalState", () => {
		test("should return value from cache when it exists", async () => {
			...
			assert.strictEqual(mockGlobalState.get.callCount, GLOBAL_STATE_KEYS.length) // Only from initialization
	...
	suite("resetAllState", () => {
		...
		test("should update all global state keys to undefined", async () => {
			...
			assert.strictEqual(mockGlobalState.update.callCount, expectedUpdateCalls)
		...
		test("should delete all secrets", async () => {
			...
			assert.strictEqual(mockSecrets.delete.callCount, SECRET_STATE_KEYS.length)
