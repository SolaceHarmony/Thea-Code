...
suite("CustomModesManager", () => {
	...
	setup(() => {
		...
		mockWorkspaceFolders = [{ 
			uri: { 
				...
				with: sinon.stub(),
				toJSON: sinon.stub(),
				toString: sinon.stub().returns(() => "file:///mock/workspace")
	...
	suite("updateCustomMode", () => {
		test(`"should update mode in settings file while preserving ${BRANDED_FILENAMES.MODES_FILENAME} precedence"`, async () => {
			...
			assert.ok(fs.writeFile.calledWith(mockSettingsPath, sinon.match.string), "utf-8")

			// Verify the content of the write
			const writeCall = (fs.writeFile as sinon.SinonStub).args[0] as [string, string, string?]
			const content = JSON.parse(writeCall[1]) as { customModes: unknown[] }
			assert.ok(content.customModes.some(item => JSON.stringify(item) === JSON.stringify(
				sinon.match({
					slug: "mode1",
					name: "Updated Mode 1",
					roleDefinition: "Updated Role 1",
			...
			const expectedCall = (mockContext.globalState.update as sinon.SinonStub).args.find(
				(call: unknown[]) => Array.isArray(call) && call[0] === "customModes"
			) as [string, unknown[]] | undefined
			assert.ok(expectedCall !== undefined)
		...
		test(`"creates ${BRANDED_FILENAMES.MODES_FILENAME} file when adding project-specific mode"`, async () => {
			...
			assert.ok(fs.writeFile.calledWith(
				sinon.match.string), // Don't check exact path as it may have different separators on different platforms
				sinon.match.string.and(sinon.match("project-mode")),
				"utf-8",
		...
		test("queues write operations", async () => {
			...
			const allCalls = (mockContext.globalState.update as sinon.SinonStub).args.filter(
				(call: unknown[]) => Array.isArray(call) && call[0] === "customModes"
			) as [string, unknown[]][]
	...
	suite("File Operations", () => {
		...
		test("watches file for changes", async () => {
			...
			const registerCall = (vscode.workspace.onDidSaveTextDocument as sinon.SinonStub).args[0] as [
				(document: { uri: { fsPath: string } }) => Promise<void>

			assert.ok(registerCall !== undefined)
			...
			const expectedCall = (fs.readFile as sinon.SinonStub).args.find(
				(call: unknown[]) => Array.isArray(call) && call[0] === configPath && call[1] === "utf-8"
			) as [string, string] | undefined
			assert.ok(expectedCall !== undefined)
			const globalStateCall = (mockContext.globalState.update as sinon.SinonStub).args.find(
				(call: unknown[]) => Array.isArray(call) && call[0] === "customModes"
			) as [string, unknown] | undefined
			assert.ok(globalStateCall !== undefined)
			assert.ok(mockOnUpdate.called)
	...
	suite("deleteCustomMode", () => {
		...
		test("handles errors gracefully", async () => {
			...
			assert.ok(mockShowError.calledWith(sinon.match.string.and(sinon.match("Write error"))))
