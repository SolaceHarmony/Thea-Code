...
suite("CustomModesSettings", () => {
	...
	suite("type inference", () => {
		test("inferred type includes all required fields", () => {
			...
			assert.ok(settings.customModes[0].slug !== undefined)
			assert.ok(settings.customModes[0].name !== undefined)
			assert.ok(settings.customModes[0].roleDefinition !== undefined)
			assert.ok(settings.customModes[0].groups !== undefined)
		...
		test("inferred type allows optional fields", () => {
			...
			// TypeScript compilation will fail if the type is incorrect
			assert.ok(settings.customModes[0].customInstructions !== undefined)
