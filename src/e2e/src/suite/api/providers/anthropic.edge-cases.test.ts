...
/*
=> {
	const actual = // TODO: requireActual("../base-provider")
	return {
		...actual,
...
suite("AnthropicHandler - Edge Cases", () => {
	...
	let mockClient: sinon.SinonStubStatic<NeutralAnthropicClient>
	...
	setup(() => {
		...
		(NeutralAnthropicClient as sinon.SinonStub).callsFake(() => mockClient)
	...
	suite("Model Selection Edge Cases", () => {
		...
		test("should handle missing API key", () => {
			...
			assert.ok(NeutralAnthropicClient.calledWith({
				apiKey: "",
