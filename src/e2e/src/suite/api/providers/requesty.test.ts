...
suite("RequestyHandler", () => {
	...
	setup(() => {
		...
		;(OpenAI as sinon.SinonStubStaticClass<typeof OpenAI>).callsFake(
			() =>
				({
					chat: {
						completions: {
			...
			test("should not include max_tokens when includeMaxTokens is false", async () => {
				...
				assert.ok(mockCreate.calledWith(
					expect.not.objectContaining({
						max_tokens: sinon.match.number), // eslint-disable-line @typescript-eslint/no-unsafe-assignment
					}),
	...
	suite("completePrompt", () => {
		...
		test("should complete prompt successfully", async () => {
			...
			assert.ok(mockCreate.calledWith({
				...
				max_tokens: sinon.match.number), // eslint-disable-line @typescript-eslint/no-unsafe-assignment
				temperature: sinon.match.number, // eslint-disable-line @typescript-eslint/no-unsafe-assignment
