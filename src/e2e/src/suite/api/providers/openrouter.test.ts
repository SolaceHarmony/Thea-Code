...
suite("OpenRouterHandler", () => {
	...
	test("createMessage handles API errors", async () => {
		...
		sinon.spy(OpenAI.prototype.chat.completions, "create").callsFake(
			() => {
				return mockStream as unknown as ReturnType<typeof OpenAI.prototype.chat.completions.create>
	...
	test("completePrompt returns correct response", async () => {
		...
		const mockCreate = sinon.spy(OpenAI.prototype.chat.completions, "create").resolves(mockResponse)
	...
	test("completePrompt handles API errors", async () => {
		...
		sinon.spy(OpenAI.prototype.chat.completions, "create").rejects(mockError)
	...
	test("createMessage processes OpenAI tool calls", async () => {
		...
		assert.ok(chunks.some(item => JSON.stringify(item) === JSON.stringify({ type: "tool_result", id: "call1", content: { result: "ok" } })))
