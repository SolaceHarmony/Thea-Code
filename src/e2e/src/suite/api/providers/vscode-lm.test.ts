...
=> {
	...
	return {
		...
		CancellationTokenSource: sinon.stub().returns(() => ({
			token: {
		...
		LanguageModelChatMessage: {
			Assistant: sinon.stub().returns((content: string | MockLanguageModelPart[]) => ({
				role: "assistant",
			...
			User: sinon.stub().returns((content: string | MockLanguageModelPart[]) => ({
				role: "user",
				content: Array.isArray(content) ? content : [new MockLanguageModelTextPart(content as string)], // eslint-disable-line @typescript-eslint/no-unnecessary-type-assertion
			})),
		},
...
suite("VsCodeLmHandler", () => {
	...
	suite("constructor", () => {
		...
		test("should handle configuration changes", () => {
			...
			const callArgs = mockOnDidChangeConfiguration.args[0] as [(e: vscode.ConfigurationChangeEvent) => void]
	...
	suite("createClient", () => {
		test("should create client with selector", async () => {
			...
			;(vscode.lm.selectChatModels as sinon.SinonStub).resolvesOnce([mockModel])
		...
		test("should return default client when no models available", async () => {
			;(vscode.lm.selectChatModels as sinon.SinonStub).resolvesOnce([])
	...
	suite("createMessage", () => {
		...
		test("should stream text responses", async () => {
			...
			mockLanguageModelChat.sendRequest.resolvesOnce({
				stream: (function* () {
			...
			expect(chunks[1]).toMatchObject({
				type: "usage",
				inputTokens: sinon.match.number, // eslint-disable-line @typescript-eslint/no-unsafe-assignment
				outputTokens: sinon.match.number, // eslint-disable-line @typescript-eslint/no-unsafe-assignment
		...
		test("should handle errors", async () => {
			...
			mockLanguageModelChat.sendRequest.rejectsOnce(new Error("API Error"))
	...
	suite("completePrompt", () => {
		...
		test("should handle errors during completion", async () => {
			...
			mockLanguageModelChat.sendRequest.rejectsOnce(new Error("Completion failed"))
