...
suite("UnboundHandler", () => {
		...
		test("should handle streaming responses with text and usage data", async () => {
			...
			assert.ok(mockCreate.calledWith(
				sinon.match({
					...
					messages: sinon.match.array), // eslint-disable-line @typescript-eslint/no-unsafe-assignment
					stream: true,
				}),
				sinon.match({
					headers: {
						"X-Unbound-Metadata": sinon.match.string.and(sinon.match(EXTENSION_NAME)), // eslint-disable-line @typescript-eslint/no-unsafe-assignment
					},
				}),
	...
	suite("completePrompt", () => {
		test("should complete prompt successfully", async () => {
			...
			const [params, options] = mockCreate.args[0]
		...
		test("should not set temperature for openai/o3-mini", async () => {
			...
			const headers = options?.headers as Record<string, string>
			assert.ok(headers["X-Unbound-Metadata"].includes(EXTENSION_NAME))
			expect(mockCreate.args[0][0]).not.toHaveProperty("temperature")
