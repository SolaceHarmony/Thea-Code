...
=> {
	...
	return {
		__esModule: true,
		default: sinon.stub().returns(() => mockClient),
		AzureOpenAI: sinon.stub().returns(() => mockClient)
...
// TODO: Implement proper mock with proxyquire
	...
	convertToOpenAiContentBlocks: sinon.stub().returns((content: string | NeutralMessageContent) => {
...
suite("OpenAiHandler - Edge Cases", () => {
	...
	let mockClient: sinon.SinonStubStatic<OpenAI>
	let mockOptions: ApiHandlerOptions
	let consoleWarnSpy: sinon.SinonSpy<void, [message?: unknown, ...optionalParams: unknown[]]>
	let mockXmlMatcher: sinon.SinonStubStatic<XmlMatcher>
	...
	setup(() => {
		...
		const OpenAIMock = // TODO: requireMock("openai").default
		...
		} as unknown as sinon.SinonStubStatic<XmlMatcher>
	...
	suite("Streaming vs Non-Streaming", () => {
		test("should handle streaming mode with XmlMatcher for reasoning", async () => {
			...
			mockXmlMatcher.update.returnsOnce([
			...
			assert.ok(results.some(item => JSON.stringify(item) === JSON.stringify({
				type: "reasoning",
				text: "More reasoning"
			})))
	...
	suite("Tool Calls to MCP Conversion", () => {
		test("should convert tool_calls to MCP tool_result", async () => {
			...
			// TODO: requireMock("../shared/tool-use").ToolCallAggregator.callsFake(() => mockAggregator)
		...
		test("should handle multiple tool calls in sequence", async () => {
			...
			const mockAggregator = {
				...
				getCompletedToolCalls: sinon.stub()
					.returnsOnce([{ id: "call_1", name: "tool1", arguments: {} }])
					.returnsOnce([{ id: "call_2", name: "tool2", arguments: {} }])
	...
	suite("XmlMatcher Reasoning Extraction", () => {
		test("should extract thinking tags from content", async () => {
			...
			assert.ok(results.some(item => JSON.stringify(item) === JSON.stringify({ type: "thinking", text: "Let me analyze this" })))
			assert.ok(results.some(item => JSON.stringify(item) === JSON.stringify({ type: "text", text: "The answer is 42" })))
		...
		test("should call XmlMatcher.final() for remaining content", async () => {
			...
			assert.ok(results.some(item => JSON.stringify(item) === JSON.stringify({ type: "thinking", text: "incomplete" })))
	...
	suite("Azure OpenAI Handling", () => {
		test("should detect Azure URLs and use AzureOpenAI client", () => {
			...
			const { AzureOpenAI } = // TODO: requireMock("openai")
	...
	suite("Model-Specific Handling", () => {
		test("should handle models without temperature support", async () => {
			...
			assert.ok(results.some(item => JSON.stringify(item) === JSON.stringify({ type: "text", text: "O3 response" })))
