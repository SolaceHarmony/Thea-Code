...
interface SearchFilesDependencies {
	path: {
		getReadablePath: typeof pathUtils.getReadablePath
	}
	ripgrep: {
		regexSearchFiles: typeof ripgrep.regexSearchFiles
	}
}
...
export async function searchFilesTool(
	...
	dependencies: SearchFilesDependencies = {
		path: { getReadablePath: pathUtils.getReadablePath },
		ripgrep: { regexSearchFiles: ripgrep.regexSearchFiles },
	}
) {
	...
	const sharedMessageProps: TheaSayTool = {
		tool: "searchFiles",
		path: dependencies.path.getReadablePath(theaTask.cwd, removeClosingTag("path", relDirPath)),
	...
	try {
		if (block.partial) {
			...
			const results = await dependencies.ripgrep.regexSearchFiles(
				theaTask.cwd,
				absolutePath,
				regex,
				filePattern,
