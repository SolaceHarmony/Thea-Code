...
suite("TheaMcpManager", () => {
	...
	let mockMcpHub: sinon.SinonStubbedInstance<McpHub>
	...
	setup(() => {
		...
		} as unknown as sinon.SinonStubbedInstance<McpHub>
	...
	suite("McpHub fallback behavior when not set", () => {
		test("updateServerTimeout logs warning when McpHub not set", async () => {
			...
			assert.ok(console.warn.calledWith(
				// TODO: String contains check - "McpHub not available for updateServerTimeout")),
			)
		})
		...
		test("deleteServer logs warning when McpHub not set", async () => {
			...
			// Verify
			assert.ok(console.warn.calledWith(// TODO: String contains check - "McpHub not available for deleteServer")))
		})
		...
		test("toggleServerDisabled logs warning when McpHub not set", async () => {
			...
			// Verify
			assert.ok(console.warn.calledWith(
				// TODO: String contains check - "McpHub not available for toggleServerDisabled")),
			)
		})
		...
		test("restartConnection logs warning when McpHub not set", async () => {
			...
			// Verify
			assert.ok(console.warn.calledWith(
				// TODO: String contains check - "McpHub not available for restartConnection")),
			)
		})
		...
		test("getMcpSettingsFilePath uses fallback path when McpHub not set", async () => {
			...
			// Verify
			assert.ok(console.warn.calledWith(
				// TODO: String contains check - "McpHub not available for getMcpSettingsFilePath")),
			)
