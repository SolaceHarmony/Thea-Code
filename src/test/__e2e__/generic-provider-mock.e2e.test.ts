...
suite('Generic Provider Mock', () => {
	...
	suiteSetup(async function () {
		this.timeout(20_000)
		const serverModule = await import('../../../test/generic-provider-mock/server')
		GenericProviderMock = serverModule.default
		PROVIDER_CONFIGS = serverModule.PROVIDER_CONFIGS

		const helpersModule = await import('../../../test/generic-provider-mock/test-helpers')
	...
	suite('Test Helper Integration', () => {
		test('should work with test helper', async () => {
			...
			try {
				...
				assert.ok(/http:\/\/127\.0\.0\.1:\d+/.test(url))
