...
- Removed remaining central duplicates that now exist as co-located specs under src/__e2e__/:
  - src/e2e/src/suite/workspace-edit-insert.test.ts
  - src/e2e/src/suite/fs-write-read-node.test.ts
  ounts) to avoid oversized IPC payloads; append truncation notice.
- Errors: Wrap thrown errors in `launchBrowser` and `doAction` with safe messages using `getErrorMessage(err)` and preserve stack.
- Acceptance: No API changes; e2e still passes; added small unit tests for these branches.

5) Decide on JS build script posture (and document it)
- Option A (quick): Keep `esbuild.js` in JS; ensure JS-only ESLint profile applies (it does in `eslint.config.mjs`). Add any essential JS rules you care about (already: `no-undef`, `no-import-assign`).
- Option B (typed): Migrate `esbuild.js` → `esbuild.ts`, wire a tiny `ts-node/tsx` or prebuild step.
- Acceptance: `npm run build` works the same; docs mention the decision.

6) Solidify local lint/typing metrics in workflow docs
- Capture a local baseline if you want to track drift:
  - `npm run lint:baseline` (writes `lint-output/eslint-baseline.json`)
  - `npm run lint:compare-baseline`
- Run typing metric:
  - `npm run metrics:any` (writes `lint-output/any-usage.json`)
- Update `docs/FIX_LINTER.md` and `CONTRIBUTING.md` with a short “Local metrics” snippet and where artifacts are written.
- Acceptance: Docs reflect the non-CI policy and local commands.

---

### Coming right after (near-term)
- Performance guardrail: add a cheap size check to reject screenshots larger than a threshold (configurable) to avoid memory spikes on full-page captures.
- Webview UI quick pass: ensure `react-hooks` rules and props typing are clean in frequently-touched components.

---

### Suggested PR sequencing
- PR A: BrowserSession tests (reuse + reload) and try/finally listener cleanup; small error-handling polish. No external API changes.
- PR B: Jest → assert/strict + sinon conversions in any remaining suites.
- PR C: Targeted unit tests for `path-mentions` and `shared/modes`.
- PR D: Optional: JS build script decision (documented), minor doc updates for local lint/metrics.
