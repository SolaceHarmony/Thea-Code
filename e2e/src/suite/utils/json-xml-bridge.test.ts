...
suite("json-xml-bridge", () => {
	...
	suite("JsonMatcher", () => {
		test("should detect and extract JSON thinking blocks", () => {
		...
		test("should handle text before and after JSON blocks", () => {
			const matcher = new JsonMatcher("thinking")
			...
			assert.deepStrictEqual(results[0], { matched: false, data: "Text before " })
			assert.deepStrictEqual(results[1], { matched: true, data: "Reasoning", type: "thinking" })
			assert.deepStrictEqual(results[2], { matched: false, data: " text after" })
		})

		test("should handle streaming JSON in multiple chunks", () => {
			const matcher = new JsonMatcher("thinking")
			...
			assert.deepStrictEqual(results1[0], { matched: false, data: "Text before " })
			...
			assert.deepStrictEqual(reasoningItem, { matched: true, data: "Reasoning", type: "thinking" })

			// Find the text after item
			const textAfterItem = results2.find((item) => !item.matched && item.data === " text after")
			assert.deepStrictEqual(textAfterItem, { matched: false, data: " text after" })
		})
		...
		test("should handle final method with remaining content", () => {
			const matcher = new JsonMatcher("thinking")
			...
			assert.ok(finalResults.length > 0)
			...
			assert.notStrictEqual(textAfterItem, undefined)
		})

		test("should handle non-matching JSON objects", () => {
			const matcher = new JsonMatcher("thinking")
	...
	suite("FormatDetector", () => {
		...
		test("should return unknown for ambiguous or plain text", () => {
	...
	suite("HybridMatcher", () => {
		test("should handle XML content", () => {
			...
			const mockXmlUpdate = sinon.stub().returns([
			...
			assert.strictEqual(results.length, 3)
			assert.deepStrictEqual(results[0], { matched: false, data: "Text before " })
			assert.deepStrictEqual(results[1], { matched: true, data: "This is reasoning", type: "thinking" })
			assert.deepStrictEqual(results[2], { matched: false, data: " text after" })
		})
		...
		test("should handle format detection with multiple chunks", () => {
			const matcher = new HybridMatcher("think", "thinking")
			...
			assert.deepStrictEqual(results[0], { matched: true, data: "This is reasoning", type: "thinking" })
			assert.deepStrictEqual(results[1], { matched: false, data: " text after" })
		})
		...
		test("should handle final method with remaining content", () => {
			...
			customMatcher["formatDetector"].detectFormat = sinon.stub().returns("json")

			// Mock the jsonMatcher's final method
			const mockJsonFinal = sinon.stub().returns([
			...
			assert.ok(mockJsonFinal.called)

			// Since we're using a mock that returns exactly 2 items, we can assert on that
			assert.strictEqual(finalResults.length, 2)
			assert.deepStrictEqual(finalResults[0], { matched: true, data: "Reasoning", type: "thinking" })
			assert.deepStrictEqual(finalResults[1], { matched: false, data: " text after" })
		})

		suite("HybridMatcher with Tool Use and Tool Result", () => {
			test("should handle XML tool use with the matchToolUse option", () => {
				...
				assert.ok(toolUseIds.size > 0)
				expect(toolUseIds.has("read_file")).toBeTruthy()
			})

			test("should handle JSON tool use with the matchToolUse option", () => {
				const matcher = new HybridMatcher(
					"think",
					"thinking",
					undefined, // Use default transform
			...
			test("should handle both tool use and tool result with both options enabled", () => {
			...
			test("should still handle regular thinking blocks when tool use matching is enabled", () => {
				const matcher = new HybridMatcher(
					"think",
					"thinking",
					undefined, // Use default transform
				...
				assert.notStrictEqual(reasoningItem, undefined)
				assert.strictEqual(reasoningItem?.data, "This is reasoning")
			})
		})
	})
	suite("jsonToolUseToXml", () => {
		test("should convert JSON tool use to XML format", () => {
			const jsonObj = {
				type: "tool_use",
				name: "read_file",
				id: "read_file-123",
		...
		test("should handle non-tool-use JSON objects", () => {
			const jsonObj = { type: "other", content: "Not a tool use" }
			expect(jsonToolUseToXml(jsonObj)).toBe('{"type":"other","content":"Not a tool use"}')
		})
	})

	suite("xmlToolUseToJson", () => {
		test("should convert XML tool use to JSON format", () => {
			const xmlContent = "<read_file>\n<path>src/main.js</path>\n<start_line>10</start_line>\n</read_file>"
			const result = xmlToolUseToJson(xmlContent)
			const parsed = JSON.parse(result)

			assert.strictEqual(parsed.type, "tool_use")
			assert.strictEqual(parsed.name, "read_file")
			assert.strictEqual(parsed.input.path, "src/main.js")
			assert.strictEqual(parsed.input.start_line, "10") // Note: XML values are strings
			assert.notStrictEqual(parsed.id, undefined)
		})

		test("should handle nested XML content", () => {
			const xmlContent = "<execute_command>\n<command>npm install</command>\n</execute_command>"
			const result = xmlToolUseToJson(xmlContent)
			const parsed = JSON.parse(result)

			assert.strictEqual(parsed.type, "tool_use")
			assert.strictEqual(parsed.name, "execute_command")
			assert.strictEqual(parsed.input.command, "npm install")
			assert.notStrictEqual(parsed.id, undefined)
		})

		test("should handle content without tool use tags", () => {
			const content = "Plain text"
			expect(xmlToolUseToJson(content)).toBe("Plain text")
		})
	})

	suite("jsonToolResultToXml", () => {
		test("should convert JSON tool result to XML format", () => {
			const jsonObj = {
				type: "tool_result",
	...
	suite("xmlToolResultToJson", () => {
		test("should convert XML tool result to JSON format", () => {
			const xmlContent =
				'<tool_result tool_use_id="read_file-123" status="success">\nFile content here\n</tool_result>'
			...
			assert.strictEqual(parsed.tool_use_id, "read_file-123")
			assert.strictEqual(parsed.status, "success")
			assert.strictEqual(parsed.content.length, 1)
			assert.strictEqual(parsed.content[0].type, "text")
			assert.strictEqual(parsed.content[0].text, "File content here")
		})
		...
		test("should handle error results", () => {
			...
			assert.strictEqual(parsed.content.length, 1)
			assert.strictEqual(parsed.content[0].type, "text")
			assert.strictEqual(parsed.content[0].text, "Error occurred")
			assert.notStrictEqual(parsed.error, undefined)
			assert.strictEqual(parsed.error.message, "File not found")
			assert.deepStrictEqual(parsed.error.details, { code: "ENOENT" })
		})

		test("should handle content without tool result tags", () => {
			const content = "Plain text"
			expect(xmlToolResultToJson(content)).toBe("Plain text")
		})
	})

	suite("openAiFunctionCallToNeutralToolUse", () => {
		test("should convert OpenAI function call to neutral tool use format", () => {
			...
			assert.strictEqual(result?.input.path, "src/main.js")
			assert.strictEqual(result?.input.start_line, 10)
		})

		test("should handle OpenAI tool calls array", () => {
			const openAiFunctionCall = {
				tool_calls: [
					{
						id: "call_abc123",
		...
		test("should handle invalid JSON arguments", () => {
			...
			assert.strictEqual(result?.name, "read_file")
			assert.strictEqual(result?.id, "call_abc123")
			assert.strictEqual(result?.input.raw, "invalid json")
		})
	})

	suite("neutralToolUseToOpenAiFunctionCall", () => {
		test("should convert neutral tool use to OpenAI function call format", () => {
			...
			assert.strictEqual(result?.function_call?.id, "read_file-123")
			assert.strictEqual(result?.function_call?.name, "read_file")
		...
		test("should handle non-tool-use objects", () => {
			const nonToolUse = {
	...
	suite("ToolUseMatcher", () => {
		test("should detect and extract XML tool use blocks", () => {
		...
		test("should detect and extract JSON tool use blocks", () => {
			const matcher = new ToolUseMatcher()
	...
	suite("ToolResultMatcher", () => {
		test("should detect and extract XML tool result blocks", () => {
			...
			assert.notStrictEqual(toolResultItem, undefined)
		...
		test("should detect and extract JSON tool result blocks", () => {
			// Create a map of tool use IDs
			const toolUseIds = new Map<string, string>()
			toolUseIds.set("read_file", "read_file-123")
