...
suite("Cost Utility", () => {
	...
	suite("calculateApiCostAnthropic", () => {
		...
		test("should calculate basic input/output costs correctly", () => {
		...
		test("should handle cache writes cost", () => {
			...
			// Check if cost is approximately 0.018 (within 6 decimal places)
			assert.ok(Math.abs(cost - 0.018) < 0.000001, `Expected cost to be close to 0.018, got ${cost}`)
		})

		test("should handle cache reads cost", () => {
		...
		test("should handle all cost components together", () => {
		...
		test("should handle undefined cache values", () => {
			const cost = calculateApiCostAnthropic(mockModelInfo, 1000, 500)
		...
		test("should handle missing cache prices", () => {
