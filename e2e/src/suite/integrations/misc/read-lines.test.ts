...
suite("nthline", () => {
	...
	suite("readLines function", () => {
		test("should read lines from start when from_line is not provided", async () => {
			const lines = await readLines(testFile, 2)
			assert.deepStrictEqual(lines, ["Line 1", "Line 2", "Line 3"].join("\n"))
		})

		test("should read a range of lines from a file", async () => {
			const lines = await readLines(testFile, 3, 1)
			assert.deepStrictEqual(lines, ["Line 2", "Line 3", "Line 4"].join("\n"))
		})

		test("should read lines when to_line equals from_line", async () => {
			const lines = await readLines(testFile, 2, 2)
			assert.deepStrictEqual(lines, "Line 3")
		})

		test("should throw error for negative to_line", async () => {
			await expect(readLines(testFile, -3)).rejects.toThrow(
				"Invalid endLine: -3. Line numbers must be non-negative integers.",
			)
		})

		test("should throw error for negative from_line", async () => {
			await expect(readLines(testFile, 3, -1)).rejects.toThrow(
				"Invalid startLine: -1. Line numbers must be non-negative integers.",
			)
		})

		test("should throw error for non-integer line numbers", async () => {
		...
		test("should throw error when from_line > to_line", async () => {
			await expect(readLines(testFile, 1, 3)).rejects.toThrow(
				"startLine (3) must be less than or equal to endLine (1)",
			)
		})

		test("should return partial range if file ends before to_line", async () => {
			const lines = await readLines(testFile, 15, 8)
			assert.deepStrictEqual(lines, ["Line 9", "Line 10"].join("\n"))
		})

		test("should throw error if from_line is beyond file length", async () => {
