...
// 	const actualVscode: typeof vscode = require("vscode")
	...
	actualVscode.Uri.parse = sinon.stub((value: string) => actualVscode.Uri.parse(value))
	actualVscode.Uri.joinPath = sinon.stub((uri: vscode.Uri, ...paths: string[]) =>
		actualVscode.Uri.joinPath(uri, ...paths),
	)
	actualVscode.Uri.from = sinon.stub(
...
suite("CodeActionProvider", () => {
	...
	setup(() => {
		...
		mockDocument = {
			...
			lineCount: 10,
			getText: sinon.stub(() => "mocked text"), // Removed unused 'range' parameter
