...
suite("CustomModesSettings", () => {
	...
	suite("type inference", () => {
		test("inferred type includes all required fields", () => {
			...
			assert.notStrictEqual(settings.customModes[0].slug, undefined)
			assert.notStrictEqual(settings.customModes[0].name, undefined)
			assert.notStrictEqual(settings.customModes[0].roleDefinition, undefined)
			assert.notStrictEqual(settings.customModes[0].groups, undefined)
		})
