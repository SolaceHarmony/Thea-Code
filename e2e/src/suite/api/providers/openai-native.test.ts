...
suite("OpenAiNativeHandler", () => {
	...
	suite("createMessage", () => {
		...
		test("should handle missing content in response for o1 model", async () => {
	...
	suite("getModel", () => {
		test("should return model info", () => {
			...
			assert.strictEqual(modelInfo.info.maxTokens, 16384)
		...
		test("should handle undefined model ID", () => {
			...
			assert.strictEqual(modelInfo.id, "gpt-4o") // Default model
			assert.notStrictEqual(modelInfo.info, undefined)
		})
	})
})
